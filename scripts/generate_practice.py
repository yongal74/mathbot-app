...
⚠️ 수식 표기 규칙: LaTeX($, \\frac, \\sqrt 등) 절대 사용 금지. 일반 텍스트로만 작성.
   예) x²+2x-1, √(x+1), (a+b)/2, log₂(x), |x-1|
...
def generate_explanation(concept: str, subject: str) -> dict:
    """Sonnet으로 스토리 기반 개념 설명 생성"""
    prompt = f"""당신은 세계 최고의 수학 스토리텔러입니다. 한국 {subject} 과목 '{concept}' 개념을 이야기로 풀어 설명해주세요.

⚠️ 수식 표기: LaTeX($, \\frac 등) 절대 사용 금지. 일반 텍스트로만 (예: x², √2, a/b).

JSON만 응답:
{{
  "analogy": "일상 속 스토리나 상황으로 개념을 이해시키기 (3-4문장, '왜 이 개념이 생겼는지' 맥락 포함, 고등학생이 '아, 그래서!'라고 할 수 있는 이야기)",
  "explain": [
    "핵심 원리 1 — 개념의 정의를 한 줄로 (공식 포함)",
    "핵심 원리 2 — 왜 이렇게 되는지 이유/성질",
    "핵심 원리 3 — 자주 혼동하는 부분과 주의사항",
    "핵심 원리 4 — 다른 개념과의 연결고리"
  ],
